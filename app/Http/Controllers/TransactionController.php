    ...
    public function index(Auth $auth)
    {
        ...
        return app('datatables')->eloquent($model)
            ...
            ->addColumn('action', function (Transaction $item) {
                return implode([
                    Html::el('a', ["onclick"=>"hotLink('/transactions/invoice/".$item->id."')", "href"=>"transactions/invoice/".$item->id])->title(__('View Invoice'))->setHtml(Html::el('i')->class('ri-file-list-2-fill ri-2x text-primary'))
                ]);
            })
